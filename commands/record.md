...
allowed-tools: ["mcp__oneagent__oneagent_record", "mcp__oneagent__oneagent_stop", "mcp__oneagent__oneagent_status", "mcp__oneagent__browser_snapshot", "mcp__oneagent__browser_navigate", "mcp__oneagent__browser_click", "mcp__oneagent__browser_type", "mcp__oneagent__browser_fill_form"]
---

The user wants to record a browser action to generate a replayable API skill.

1. First check `mcp__oneagent__oneagent_status` to confirm there is an active explore session.
   - If no session is active, tell the user to run `/oneagent:explore <url>` first.

2. Parse the arguments:
   - First argument is the action name (required)
   - Any `key=value` pairs after `--input` become the inputs map

3. Call `mcp__oneagent__oneagent_record` with the action name and any inputs.
...
5. When the user indicates they are done (or says "stop"), call `mcp__oneagent__oneagent_stop` to:
   - Stop recording
