 ...
 */
export class PlaywrightMcpAdapter implements BrowserProvider {
  private page: Page;
  ...
  /**
   * Validate that a tool name is on the allowlist.
   * Throws if the tool is blocked or unknown.
   */
   ...
   * This is the internal dispatch — external callers use the typed methods.
   */
  async proxyTool(toolName: string, args: Record<string, unknown>): Promise<unknown> {
    ...
    switch (toolName) {
      ...
      case 'browser_handle_dialog':
        // Dialog handling is set up via page.on('dialog')
  ...
  async snapshot(): Promise<PageSnapshot> {
    ...
    // Get the accessibility tree as the primary content representation.
    // Use aria snapshot which is the supported Playwright API.
    let content = '';
    try {
      content = await this.page.locator('body').ariaSnapshot();
    } catch {
      // Fall back to inner text if aria snapshot is unavailable
      try {
  ...
  async click(ref: string): Promise<void> {
    // Try data-ref attribute first, then fall back to text or CSS selector
  ...
  async evaluateFetch(req: SealedFetchRequest): Promise<SealedFetchResponse> {
    ...
    // Execute the fetch in the page context using a sealed template.
    // The template is generated here — agents never see or control the
    // evaluate call.
  ...
  /**
   * Validate that a URL's domain is on the allowlist.
  ...
  private assertDomainAllowed(url: string): void {
    ...
    if (this.domainAllowlist.length === 0) {
      // No allowlist configured means all domains are denied for sealed fetch
      throw new Error(
  ...
  /**
   * Set up passive network request capture for networkRequests().
   */
  private setupNetworkCapture(): void {
    this.page.on('response', async (response: PwResponse) => {
      ...
      try {
        ...
        // Some responses can't be read (e.g. binary, redirects)
      }
  ...
  /**
   * Fill a form by mapping ref→value pairs.
   */
